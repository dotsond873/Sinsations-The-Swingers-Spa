...
export default function DashboardPage({ user: propUser }) {
  ...
  const nearbyLabel = user.area_code ? 'area code ' + user.area_code : user.state ? user.state : '';
    ...
    <div className="min-h-screen bg-[#0B0A0F]">
      ...
      <main className="max-w-7xl mx-auto px-8 py-12">
        ...
        {isProfileIncomplete && (
          <div data-testid="complete-profile-banner" className="mb-8 glass-effect p-6 rounded-2xl border-2 border-[#D4AF37]">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <button data-testid="complete-profile-btn" onClick={() => navigate('/profile-setup')} className="px-8 py-3 rounded-full bg-[#D4AF37] text-[#0B0A0F] font-semibold hover:bg-[#F0C847] transition-all duration-300 flex items-center justify-center gap-2 whitespace-nowrap">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#A8A3B2] text-sm">Without one you cannot reset your password if you forget it.</p>
                </div>
              </div>
              <button data-testid="set-security-question-btn" onClick={() => navigate('/profile-setup')} className="px-6 py-3 rounded-full bg-[#B22234] text-[#F7F5F0] font-semibold hover:bg-[#D62839] transition-all whitespace-nowrap">
                Set It Now
              </button>
            </div>
          </div>
        ...
        {!user.is_verified && (
          <div data-testid="get-verified-banner" className="mb-8 glass-effect p-6 rounded-2xl border-2 border-[#D4AF37]">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <button data-testid="get-verified-btn" onClick={() => navigate('/verification')} className="px-6 py-3 rounded-full bg-[#D4AF37] text-[#0B0A0F] font-semibold hover:bg-[#F0C847] transition-all whitespace-nowrap">
                Get Verified
              </button>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
          ...
          <QuickAction icon={<Chat size={32} weight="duotone" />} title="Messages" description="Chat freely" onClick={() => navigate('/messages')} testId="messages-card" />
        ...
        {nearbyMembers.length > 0 && (
          <div data-testid="nearby-members" className="mb-12">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {nearbyMembers.map(m => (
                <div key={m.user_id} onClick={() => navigate('/profile/' + m.user_id)} className="glass-effect p-4 rounded-2xl cursor-pointer hover:-translate-y-1 transition-all flex items-center gap-3">
                  {m.picture ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(m.city || m.state) && (
                      <p className="text-[#A8A3B2] text-xs truncate">{[m.city, m.state].filter(Boolean).join(', ')}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="glass-effect p-6 rounded-2xl mb-8 border border-[#D4AF37]">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button onClick={() => navigate('/support-us')} className="px-6 py-2 rounded-full bg-[#D4AF37] text-[#0B0A0F] font-semibold hover:bg-[#F0C847] transition-all whitespace-nowrap">Support Us</button>
          </div>
        </div>

        <div className="glass-effect p-6 rounded-2xl text-center">
          <p className="text-[#A8A3B2]">Remember: <span className="text-[#D4AF37] font-semibold">Be nice. Be respectful. Have fun.</span></p>
        </div>
...
function QuickAction({ icon, title, description, onClick, testId }) {
  return (
    <div data-testid={testId} onClick={onClick} className="glass-effect p-6 rounded-2xl cursor-pointer transition-all duration-300 hover:-translate-y-1">
